...
import {
  ...
  resetClaudeSessionCatalogCacheForTest,
...
describe("Claude session catalog", () => {
  ...
  it("rescans a transcript after a permission change that keeps content unchanged", async () => {
    const home = await createHome();
    const projectDir = path.join(home, ".claude", "projects", "-workspace");
    const permPath = path.join(projectDir, "perm-session.jsonl");
    await writeProject({
      home,
      entries: [],
      transcripts: { "perm-session": [sdkCliMessage("perm-session", "Kept")] },
    });
    const resolvedPermPath = await fs.realpath(permPath);
    expect((await listLocalClaudeSessionPage({}, home)).sessions).toHaveLength(1);

    // A permission change bumps ctime without touching content (mtime/size); the cache must not
    // keep serving a file that may no longer be readable.
    const before = await fs.stat(permPath);
    await fs.chmod(permPath, 0o600);
    const after = await fs.stat(permPath);
    ...
    expect(openSpy.mock.calls.map(([filePath]) => filePath)).toContain(resolvedPermPath);
    openSpy.mockRestore();
  });
  ...
  it("clears cached transcript metadata through the test reset hook", async () => {
    const home = await createHome();
    await writeProject({
      home,
      entries: [],
      transcripts: { "reset-session": [sdkCliMessage("reset-session", "Reset")] },
    });
    const openSpy = vi.spyOn(fs, "open");
    await listLocalClaudeSessionPage({}, home);
    openSpy.mockClear();
